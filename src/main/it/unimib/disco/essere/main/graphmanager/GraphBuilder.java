...
public class GraphBuilder
{
    ...
    private Map<String, Vertex> classVertexMap; // Modded (reduce graph traversals)
    private Map<String, Vertex> packageVertexMap; // Modded (reduce graph traversals)
    ...
    public GraphBuilder(HashMap<String, JavaClass> classes, HashMap<String, String> packages)
    {
        ...
        _classes = classes;
        _packages = packages;
        classVertexMap = new HashMap<>((int) (_classes.size() * SAFETY_FACTOR));
        packageVertexMap = new HashMap<>((int) (_packages.size() * SAFETY_FACTOR));
        edgeMaps = new EdgeMaps();
    }

    public GraphBuilder(HashMap<String, JavaClass> classes, HashMap<String, String> packages,
                        boolean asTdEvolution, ExTimeLogger exTimeLogger, Set<String> extClasses, Set<String> extPackages)
    {
    ...
    public void createGraph(Graph graph)
    {
        ...
        for (JavaClass clazz : _classes.values())
        {
            ...
            Vertex vertex = classVertexMap.get(clazz.getClassName()); //Modded
    ...
    private void createSuperDependency(Graph graph, JavaClass clazz, Vertex callerVertex)
    {
        ...
        {
            Vertex superclassVertex = classVertexMap.get(clazz.getSuperclassName()); //Modded
            ...
            String edgeLabel = PropertyEdge.LABEL_SUPER_DEPENDENCY.toString(); //modded
            Edge edge = callerVertex.addEdge(edgeLabel, superclassVertex);
            edgeMaps.addEdgeToEdgeMaps(edge, callerVertex, superclassVertex, edgeLabel); //modded
    ...
    private void createInterfaceDependency(Graph graph, JavaClass clazz, Vertex callerVertex)
    {
        ...
        {
            Vertex interfaceVertex = classVertexMap.get(i); //Modded
            ...
            String edgeLabel = PropertyEdge.LABEL_INTERFACE_DEPENDENCY.toString(); //Modded
            Edge edge = callerVertex.addEdge(edgeLabel, interfaceVertex);
            edgeMaps.addEdgeToEdgeMaps(edge, callerVertex, interfaceVertex, edgeLabel); //Modded
    ...
    private void createMethodDependency(Graph graph, JavaClass clazz, Vertex callerVertex)
    {
        ...
        for (String referencedclassName : dependences)
        {
            ...
            Vertex referencedclassVertex = classVertexMap.get(classInfo[0]); //Modded
            ...
            {
                String edgeLabel = PropertyEdge.LABEL_CLASS_DEPENDENCY.toString(); //Modded
                Edge dependency = edgeMaps.existEdge(edgeLabel, callerVertex, referencedclassVertex);
                if (dependency == null)
                {
                    Edge edge = callerVertex.addEdge(edgeLabel, referencedclassVertex, PROPERTY_DEPEDENCY_WEIGHT.toString(), DEFAULT_NUM_DEPENDENCES);
                    edgeMaps.addEdgeToEdgeMaps(edge, callerVertex, referencedclassVertex, edgeLabel); //Modded
    ...
    void createPackageDependency(Graph graph, String packageName, Vertex callerVertex)
    {
        ...
        if (packageName != null)
        {
            ...
            Vertex packageVertex = packageVertexMap.get(packageName); //Modded
            ...
            String edgeLabel = PropertyEdge.LABEL_PACKAGE_DEPENDENCY.toString(); //Modded
            Edge edge = callerVertex.addEdge(edgeLabel, packageVertex);
            edgeMaps.addEdgeToEdgeMaps(edge, callerVertex, packageVertex, edgeLabel); //Modded
        }
    }
    ...
    protected void createAfferentCoupling(Graph graph, String packageName, Vertex callerVertex)
    {
        if (packageName != null)
        {
            ...
            if (edgeMaps.existEdge(LABEL_AFFERENCE, callerVertex, packageVertex) == null)
            {
                Edge edge = callerVertex.addEdge(LABEL_AFFERENCE, packageVertex);
                edgeMaps.addEdgeToEdgeMaps(edge, callerVertex, packageVertex, LABEL_AFFERENCE); //Modded
            }
        }
    }
    ...
    private void createEfferentCoupling(Graph graph, String packageName, Vertex efferentVertex)
    {
        if (packageName != null)
        {
            ...
            String edgeLabel = PropertyEdge.LABEL_EFFERENCE.toString(); //Modded
            if (edgeMaps.existEdge(edgeLabel, efferentVertex, packageVertex) == null)
            {
                Edge edge = efferentVertex.addEdge(edgeLabel, packageVertex);
                edgeMaps.addEdgeToEdgeMaps(edge, efferentVertex, packageVertex, edgeLabel); //Modded
            }
    ...
    {
        List<Edge> affEdges = edgeMaps.findEdgesByLabel(GraphBuilder.LABEL_AFFERENCE);
        if (affEdges != null)
        {
            for (Edge e : affEdges)
            {
                Edge belongToEdge = graph.traversal().V(e.outVertex().id()).bothE(PropertyEdge.LABEL_PACKAGE_DEPENDENCY.toString())
                    .next();
                if (edgeMaps.existEdge(LABEL_PACKAGE_AFFERENCE, belongToEdge.inVertex(), e.inVertex()) == null)
                {
                    Edge edge = belongToEdge.inVertex().addEdge(LABEL_PACKAGE_AFFERENCE, e.inVertex());
                    edgeMaps.addEdgeToEdgeMaps(edge, belongToEdge.inVertex(), e.inVertex(), LABEL_PACKAGE_AFFERENCE); //Modded
    ...
    private List<String> computeDependences(JavaClass javaClass)
    {
        ...
        for (Method m : javaClass.getMethods())
        {
            ...
            exTimeLogger.logEventStart(Event.GB_MTHD_PROCESS_INSTRUCTIONS);
            ...
            exTimeLogger.logEventEnd(Event.GB_MTHD_PROCESS_INSTRUCTIONS);
            exTimeLogger.logEventStart(Event.GB_MTHD_PROCESS_ANNOTATIONS);
            //FIXME
            ...
            exTimeLogger.logEventEnd(Event.GB_MTHD_PROCESS_ANNOTATIONS);
        }
    ...
    Vertex createNode(Graph graph, String name, String nodeType)
    {
        ...
        if (CLASS.equals(nodeType))
        {
            ...
            // Modded
            classVertexMap.put(name, vertex);
        ...
        else
        {
            ...
            // Modded
            packageVertexMap.put(name, vertex);
    ...
    // Modded
    public Map<String, Vertex> getClasses()
    {
        return classVertexMap;
    }

    // Modded
    public Map<String, Vertex> getPackages()
    {
    ...
    public EdgeMaps getEdgeMaps() {return edgeMaps;}
